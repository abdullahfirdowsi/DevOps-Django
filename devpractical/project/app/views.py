...
def db_table(request):
    mymembers = app_db.objects.all().values()
...
def db_addrecord(request):
    ...
    db = app_db(firstname = x,lastname = y)
    db.save()
    return HttpResponseRedirect(reverse('db_table'))

def db_delete(request,id):
    dele=app_db.objects.get(id=id)
    dele.delete()
    return HttpResponseRedirect(reverse('db_table'))

def db_update(request,id):
    mymember = app_db.objects.get(id=id)
